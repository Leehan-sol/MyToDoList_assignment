...
UserDefaults: 데이터 저장을 위한 가장 기초적인 방법
1. 싱글턴 객체이다.
2. Key-Value 쌍으로 데이터를 저장하는 방식이다.
3. UserDefaults는 plist에 XML 형태로 데이터가 저장되기 때문에 데이터 크기가 커지면 파일 크기가 커져서 앱이 실행될때 저장된 데이터를 모두 메모리에 올리기 때문에 메모리 성능에 악영향을 줄 수 있다.
...
4. UserDefaults의 장점은 Thread-Safe 하다는 것이다.
CoreData와 Realm은 Thread-Safe 하지 않기 때문에 멀티 스레드 환경에서 주의를 해야하지만, UserDefaults는 Thread-Safe 하기 때문에 별도의 처리가 필요 없다.
대신, plist에 데이터가 저장되기 때문에 앱이 삭제되면 데이터도 모두 삭제됩니다. 또한, 암호화해서 저장하지 않기 때문에 보안이 좋지 않다.
> 종합해보면 간단한 소규모 데이터를 쉽게 저장하고 싶을 때 UserDefaults를 사용하는 것이 좋다.
