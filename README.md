...
1. 데이터 저장을 위한 가장 기초적인 방법으로 싱글턴 객체이다.
...
# 🛠️ CoreData 
공식 문서에서 CoreData의 5가지 기능 (데이터베이스 기능을 지원하는 프레임워크) <br/>
1. 영속성(Persistence)
2. 변경사항의 Undo, Redo
3. 백그라운드 데이터 작업 기능
4. 동기화 기능
...
이 중 데이터베이스는 1번 영속성 기능이다. <br/>
이 영속성 기능은 SQLite를 기반으로 지원하는데 일반적인 데이터베이스와는 차이가 있다. <br/>
CoreData는 객체 그래프 관리자로 객체를 직접적으로 연결해서 관리한다. <br/>
데이터베이스는 행렬 관계를 이용해 데이터를 읽고 쓰는 반면 CoreData는 객체를 연속적으로 탐색해야 한다는 점이 차이점이다. <br/>
링크드 리스트처럼 객체를 탐색하다보니 탐색 성능은 비교적 나쁘고, Thread-Safe 하지 않아 Lock으로 동기화 처리를 해줘야한다. <br/>
